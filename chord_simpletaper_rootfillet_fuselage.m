function Chord = chord_simpletaper_rootfillet_fuselage(ChordParameters, Epsilon)
...
FuselageSide = ChordParameters(4);
...
if RootAftFilletRatio > 0
    ...
    AddChord = ((RootAftFilletRatio - (Epsilon-FuselageSide))/RootAftFilletRatio).^2*RootAftFilletAmplitude;
    
    AddChord(Epsilon > RootAftFilletRatio + FuselageSide) = 0;
        
    AddChord(Epsilon < FuselageSide) = max(AddChord(Epsilon <= FuselageSide));
